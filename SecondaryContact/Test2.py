...
def getTrees(N_main, N_island,  N_ancestral, T_split):
	...
	demography.add_migration_rate_change(time = 0.00000001,  rate = 0.0, source="island", dest="mainland")
	demography.add_migration_rate_change(time = 0.00000002,  rate = 0.0, source="mainland", dest="island")
...
def getvalue(N_main, N_island,  N_ancestral, T_split ):
	SFSFULL = np.zeros((24*2+1,11*2 + 1))
	for _ in range(repitations):
		SFS = np.zeros((24*2+1,11*2 + 1))
		tre = getTrees(N_main, N_island,  N_ancestral, T_split )
		totalmut = 0
...
def negloglieklihoodfunction(args):
	...
	A = getvalue(N_main, N_island,  N_ancestral, T_split )
	print(N_main, N_island,  N_ancestral, T_split, A)
...
res = minimize(negloglieklihoodfunction,  [  	log10(15380.0),  #mainland size
